    ...
    <div class="absolute right-0 top-0 h-full flex items-center">
      <select v-model="selectedOption" :disabled="isSearching"
        ...
        <optgroup label="フロントエンド">
          <option v-for="(label, value) in filterOptions.languages.frontend" :key="`frontend-${value}`" :value="value">
            {{ label }}
          </option>
        </optgroup>
        <optgroup label="バックエンド">
          <option v-for="(label, value) in filterOptions.languages.backend" :key="`backend-${value}`" :value="value">
            {{ label }}
          </option>
        </optgroup>
      </select>
